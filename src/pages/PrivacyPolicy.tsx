...
const PrivacyPolicy = () => {
  ...
  return <>
    <style>{`
      .privacy-page { background: ${PAGE_BG}; color: ${C.white}; font-family: 'Space Grotesk', 'Inter', sans-serif; }
      .privacy-page > header { background: linear-gradient(180deg, ${C.graphite}55 0%, transparent 100%); }
      .privacy-content { background: transparent; }
      ...
      .privacy-h2 { color: ${C.white}; font-family: 'Space Grotesk', 'Inter', sans-serif; font-size: clamp(1.25rem, 2.2vw, 1.5rem); font-weight: 900; line-height: 1.2; letter-spacing: -0.04em; margin: 0 0 1.25rem; }
      .privacy-p, .privacy-list { color: rgba(255,255,255,0.55); font-size: 1rem; line-height: 1.75; }
    ...
    `}</style>
    <Navbar />
    <div className="privacy-page relative min-h-screen overflow-hidden font-display">
      ...
      <header className="relative z-10 border-b px-5 pb-10 pt-28 sm:px-8 sm:pb-14 sm:pt-32 lg:px-12" style={{ borderColor: C.wa(0.07) }}>
        <div className="mx-auto max-w-[1200px]">
          ...
          <h1 className="mb-8 max-w-4xl text-5xl font-black leading-[0.96] tracking-[-0.045em] sm:text-6xl">Privacy Policy</h1>
          <p className="max-w-xl text-lg leading-8 sm:text-xl" style={{ color: C.wa(0.64) }}>We are committed to protecting personal data in accordance with GDPR and international privacy regulations.</p>
        </div>
      </header>
      <main className="privacy-content relative z-10 mx-auto max-w-[1200px] px-5 py-10 sm:px-8 sm:py-16 lg:px-12 lg:py-20">
        ...
        <div className="lg:grid lg:grid-cols-[240px_minmax(0,1fr)] lg:items-start lg:gap-16 xl:gap-20">
          ...
          <article className="privacy-doc min-w-0" style={{ maxWidth: 760 }}>
            <section id="overview" className="privacy-section" tabIndex={-1}><h2 className="sr-only">Introduction</h2><p className="privacy-p mb-4">Velnix Solutions ("Company", "we", "our", or "us") is committed to protecting the privacy and security of personal data in accordance with applicable data protection laws, including the General Data Protection Regulation (GDPR) (EU) 2016/679 and other international privacy regulations.</p><p className="privacy-p">This Privacy Policy explains how we collect, use, process, disclose, and safeguard your information when you engage with our services, website, or business operations.</p></section>
            <SectionDivider />
...
function PrivacySection({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return <section id={id} className="privacy-section" tabIndex={-1}><p className="mb-3 text-[11px] font-bold uppercase tracking-[0.24em]" style={{ color: C.lime }}>{title.split(".")[0].padStart(2, "0")}</p><h2 className="privacy-h2">{title}</h2>{children}</section>;
}
