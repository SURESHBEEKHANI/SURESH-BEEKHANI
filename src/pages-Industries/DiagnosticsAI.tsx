          ...
          <div className="flex-1 text-white space-y-8">
            <div className="w-full">
              <h1 className="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight drop-shadow-lg w-full">
                AI-Driven <span className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Diagnostics</span>
              </h1>
              <p className="text-lg md:text-2xl text-gray-100 w-full whitespace-pre-line mt-4">
                Accelerate healthcare innovation with instant, precise disease detection, automated image analysis, and actionable insights that empower professionals to deliver exceptional care.
              </p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center space-y-6 mb-16">
            ...
            <h2 className="text-3xl font-bold text-blue-900">
              Real-World AI Diagnostics Use Cases
            </h2>
            <p className="text-xl text-blue-700 max-w-3xl mx-auto">
              See how AI is reshaping diagnostics for healthcare and industry leaders worldwide
            </p>
          </div>
          <div className="relative">
            {/* Navigation Arrows */}
              ...
              {useCases.slice(currentIndex, currentIndex + 3).map((useCase) => (
                ...
                >
                  <div className="relative h-48 overflow-hidden rounded-t-xl">
                    <img
                      src={useCase.image}
                      alt={useCase.alt.replace('Analysis', 'AI-powered Analysis').replace('Detection', 'AI-driven Detection').replace('Automation', 'Automated Pathology').replace('Support', 'AI Clinical Support').replace('Optimization', 'AI Workflow Optimization').replace('Anomaly Detection in Lab Results', 'AI Anomaly Detection in Lab Results')}
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600">{
                      useCase.id === 0 ? "Leverage AI to interpret X-rays, MRIs, and CT scans—spotting tumors, fractures, and infections with unmatched accuracy and speed." :
                      useCase.id === 1 ? "Detect early warning signs of cancer, diabetes, and heart disease—empowering proactive care and better patient outcomes." :
                      useCase.id === 2 ? "Predict patient risks and emerging health issues by analyzing EHRs and lifestyle data, enabling smarter, data-driven interventions." :
                      useCase.id === 3 ? "Accelerate pathology workflows with automated slide analysis—reducing errors and delivering faster, more reliable results." :
                      useCase.id === 5 ? "Unlock insights from genetic data to identify mutations and personalize treatment strategies for every patient." :
                      useCase.id === 6 ? "Support clinicians with real-time, evidence-based recommendations—enhancing diagnostic confidence and care quality." :
                      useCase.id === 7 ? "Streamline diagnostic operations, automate routine tasks, and optimize resource allocation for maximum efficiency." :
                      useCase.id === 9 ? "Spot abnormal lab trends instantly, enabling early intervention and improved patient safety across your organization." :
                      useCase.description
                    }</p>
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-8">
            ...
            <div className="bg-white/90 rounded-2xl shadow-lg border border-blue-100 p-6 flex flex-col items-center text-center space-y-3">
              ...
              <h3 className="text-lg font-bold text-blue-900">Global Impact</h3>
              <p className="text-gray-700 text-sm">
                Delivering scalable, accessible AI diagnostics solutions to clients worldwide—empowering organizations to lead in digital health.
              </p>
            </div>
          </div>
        </div>
              ...
              >
                ...
                >
                  <span className="text-lg font-semibold text-gray-900">{
                    faq.id === 1 ? "What is AI Diagnostics?" :
                    faq.id === 2 ? "How does AI enhance diagnostic accuracy?" :
                    faq.id === 3 ? "Is AI Diagnostics safe and dependable?" :
                    faq.id === 4 ? "Will AI replace doctors in diagnostics?" :
                    faq.id === 5 ? "What are the main challenges of AI in diagnostics?" :
                    faq.question
                  }</span>
                  <svg
                ...
                >
                  <p>{
                    faq.id === 1 ? "AI Diagnostics leverages artificial intelligence and machine learning to detect, analyze, and predict diseases—enabling faster, more accurate medical decisions." :
                    faq.id === 2 ? "AI processes vast medical data, uncovers subtle patterns, and reduces human error—delivering more precise, timely diagnoses." :
                    faq.id === 3 ? "When rigorously validated and used alongside clinical expertise, AI diagnostics are highly reliable and enhance patient safety. Ongoing oversight ensures continued trust." :
                    faq.id === 4 ? "AI is a powerful support tool for healthcare professionals, not a replacement. Final decisions always remain with clinicians, ensuring the best of both human and machine intelligence." :
                    faq.id === 5 ? "Key challenges include safeguarding data privacy, seamless system integration, algorithm transparency, and ensuring fair, unbiased care for all." :
                    faq.answer
                  }</p>
                </div>
              </div>
...
const BenefitCard = ({ iconBg, iconColor, title, description, svgPath }: any) => (
  <div className="group bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 hover:border-green-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center">
    <div className={`flex-shrink-0 w-12 h-12 ${iconBg} rounded-lg flex items-center justify-center mb-4 group-hover:bg-opacity-40 transition-colors`}>
